...
function ScalarField({ disabled = false, label, onChange, step, value }: ScalarFieldProps) {
  return (
    <label className="sym-ss-field-row">
      ...
      <div className="duration-control">
        <div className="duration-inputs">
          ...
          <input
            ...
            max={1}
            min={0}
            onChange={(event) => onChange([value[0], Math.max(Number(event.currentTarget.value), value[0])])}
            step={0.01}
            type="number"
            value={value[1]}
          />
...
type SymSparseStructurePanelProps = {
  dispatch: Dispatch<SymSparseStructureAction>;
  onGenerate: () => void;
  state: SymSparseStructureState;
  symmetryTuple: SymmetryTuple;
};

export function SymSparseStructurePanel({
  dispatch,
  onGenerate,
  state,
  symmetryTuple,
}: SymSparseStructurePanelProps) {
  const running = state.status === 'running';
  const readyForNext = state.status === 'ready' && state.voxelCount > 0;

  return (
    <div className="symmetry-panel sym-ss-panel">
      ...
      <section className="symmetry-section sym-ss-generation-section">
        <IntegerStepperField
          disabled={running}
          label="seed"
