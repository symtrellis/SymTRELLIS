...
function formatVector(vector: SymmetryTuple['majorAxis']) {
...
function ScalarField({ disabled = false, label, onChange, step, value }: ScalarFieldProps) {
  return (
    <label className="sym-shape-field-row">
...
function IntegerStepperField({
  ...
  return (
    <label className="sym-shape-stepper-row">
      <span className="field-label">{label}</span>
...
type SymShapePanelProps = {
  dispatch: Dispatch<SymShapeAction>;
  onGenerate: () => void;
  state: SymShapeState;
...
export function SymShapePanel({ dispatch, onGenerate, state, symmetryTuple }: SymShapePanelProps) {
  const running = state.status === 'running';
  const readyForNext = state.status === 'ready' && Boolean(state.generatedShapeUrl);

  return (
    <div className="symmetry-panel sym-shape-panel">
      <section className="symmetry-section proposed-symmetry">
        <dl className="proposed-tuple">
          <div className="proposed-row">
            ...
            <dd>{symmetryTuple.label}</dd>
          </div>
          <div className="proposed-row">
            <dt>major axis</dt>
            <dd>{formatVector(symmetryTuple.majorAxis)}</dd>
          </div>
          <div className="proposed-row">
            <dt>minor axis</dt>
            <dd>{formatVector(symmetryTuple.minorAxis)}</dd>
          </div>
          <div className="proposed-row">
            <dt>center</dt>
            <dd>{formatVector(symmetryTuple.center)}</dd>
          </div>
        </dl>
      </section>
      ...
      <section className="symmetry-section">
        <ScalarField
          ...
          onChange={(noiseSymmetryProjectionStrength) =>
            dispatch({ params: { noiseSymmetryProjectionStrength }, type: 'paramsChanged' })
          }
          step={0.01}
          value={state.noiseSymmetryProjectionStrength}
        />
        <ScalarField
          disabled={running}
          label="symmetry projection strength"
          onChange={(symmetryProjectionStrength) =>
            dispatch({ params: { symmetryProjectionStrength }, type: 'paramsChanged' })
          }
          step={0.01}
          value={state.symmetryProjectionStrength}
        />
        <DurationRangeControl
          disabled={running}
          label="symmetry projection duration"
          onChange={(symmetryProjectionDuration) =>
            dispatch({ params: { symmetryProjectionDuration }, type: 'paramsChanged' })
          }
          value={state.symmetryProjectionDuration}
        />
      </section>

      <section className="symmetry-section sym-shape-generation-section">
        <IntegerStepperField
          disabled={running}
          label="seed"
      ...
      <section className="symmetry-section">
        <div className="sym-shape-mode-options">
          {shapeModes.map((item) => (
            <button
        ...
        {state.mode === 'cascade' ? (
          <div className="sym-shape-token-panel">
            <label className="sym-shape-token-row">
              ...
              />
              <span className="sym-shape-vram-est">
                VRAM est bf16 {estimateBf16FlowPeakGb(state.maxTokens).toFixed(2)} GB
              </span>
            </label>
            <Slider.Root
              className="sym-shape-token-slider"
              disabled={running}
              max={524288}
              min={4096}
