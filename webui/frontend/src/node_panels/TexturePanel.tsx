...
import type { DurationRange, TextureAction, TextureState } from '../state';
...
function ScalarField({ disabled = false, label, onChange, step, value }: ScalarFieldProps) {
  return (
    <label className="texture-field-row">
...
type TexturePanelProps = {
  dispatch: Dispatch<TextureAction>;
  onGenerate: () => void;
  state: TextureState;
};

export function TexturePanel({ dispatch, onGenerate, state }: TexturePanelProps) {
  const running = state.status === 'running';
  const readyForNext = state.status === 'ready' && Boolean(state.generatedTextureUrl);

  return (
    <div className="symmetry-panel texture-panel">
      <section className="symmetry-section texture-generation-section">
        <IntegerStepperField
          disabled={running}
          label="seed"
