...
NeighborGraphLatentMapperScale = Literal["tiny", "small", "base"]
...
@dataclass(frozen=True)
class NeighborGraphLatentMapperConfig:
    """Configuration for the neighbor-graph Spatial Transform Latent Mapper backend.
...
def neighbor_graph_latent_mapper_config(
    scale: NeighborGraphLatentMapperScale = "small",
    latent_dim: int = 8,
    lowrank_rank: Optional[int] = None,
) -> NeighborGraphLatentMapperConfig:
    """Return one preset neighbor-graph latent mapper config by scale name."""

    if scale == "tiny":
        return NeighborGraphLatentMapperConfig(
            feat_dim=96,
            num_heads=6,
            depth=4,
            condition_dim=96,
